...
class DplsettingController extends AdminBaseController{
	...
	public function version_control(){
		$info=Db::name('dplsetting')->where(array('type'=>'version_control'))->find();
...
// var_dump($data);die;
Db::name('dplsetting')->where(array('type'=>'version_control'))->update($data);
            // Db::name('dplsetting')->insert($data);
            $this->success("编辑成功", url("Dplsetting/version_control"));
        }
...
// var_dump($data);die;
	...
	//支付宝支付信息
	public function alipay_control(){
		$info=Db::name('dplsetting')->where(array('type'=>'alipay_control'))->find();
		if($info['content']){
			$content = json_decode($info['content'],true);
		}else{
			$content['appid'] = '';
		...
		if( $this->request->post()) {
            $data = $this->request->param();
			$data['content'] = json_encode($data['content']);
			Db::name('dplsetting')->where(array('type'=>'alipay_control'))->update($data);
            $this->success("编辑成功", url("Dplsetting/alipay_control"));
        }
	...
	//微信支付信息
	public function wechat_control(){
		$info=Db::name('dplsetting')->where(array('type'=>'wechat_control'))->find();
		if($info['content']){
			$content = json_decode($info['content'],true);
		}else{
			$content['appid'] = '';
		...
		if( $this->request->post()) {
            $data = $this->request->param();
			$data['content'] = json_encode($data['content']);
			Db::name('dplsetting')->where(array('type'=>'wechat_control'))->update($data);
            $this->success("编辑成功", url("Dplsetting/wechat_control"));
        }
