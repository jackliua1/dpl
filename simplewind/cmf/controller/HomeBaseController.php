...
hello;
    ...
    //原系统自带的检测登陆方法
    public function checkUserLogin_old()
    {
    ...
    {
        $token = isset($_REQUEST['token'])?$_REQUEST['token']:0;  //获取用户token
        $huiyuan = $this->HuiyuanModel->user_by_token($token);
        if (!$huiyuan) {
            echo return_json((object)[],-1,'请先登录');
            exit ();
        }else{
            $huiyuan['avatar'] = cmf_get_user_avatar_url($huiyuan['avatar']);
            return $huiyuan;
        }
    }
